...
// Create a minimal IssueComment object.
func testGithubComment(body string) *github.IssueComment {
	return &github.IssueComment{Body: github.Ptr(body)}
}

// Create a minimal Comment object.
func testComment(id int64) *Comment {
    return &Comment{ptr: &github.IssueComment{ID: github.Ptr(id)}}
}

// A list of comment requirements suitable for comparison against a list of
// real comment objects.
func testWantComments(ids ...int64) []*Comment {
...
func testVerifyComments(fn string, got, want []*Comment, t *testing.T) {
    ...
    if len1, len2 := len(got), len(want); len1 < len2 {
        ...
        g := map[int64]*Comment {}
        for _, comment := range got {
            if id := CommentNumber(comment); id != 0 {
                g[id] = comment
            }
        }
        for _, comment := range want {
            if id := CommentNumber(comment); id != 0 {
                if g[id] == nil {
                    t.Errorf("%s() missing comment %d", fn, id)
                }
            }
        }
    }
